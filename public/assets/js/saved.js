$(document).ready(function () {
		...
		$.ajax({
			...
			if (data.comments.length !== 0) {
				...
				for (i = 0; i < data.comments.length; i++) {
					...
					$("#comments-list").append("<li class='comment-div'><button class='delete-comment' data-id='" + data.comments[i]._id + "'>Remove</button> <label class='comment'>" + data.comments[i].body + "</label></li>");
				}
				$("#comments-list").append("</ul>");
			}
	...
	$(document).on("click", ".delete-comment", function () {
		// delete comment
		console.log('onclick .delete-comment')
		console.log(this)
	});
